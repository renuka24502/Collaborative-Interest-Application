...
out.println("<html lang='en'>");
  ...
  out.println("<body>");
    ...
    out.println("<div class='navbar navbar-default navbar-fixed-center' role='navigation' style=\"margin-bottom:0px;\">");
...
out.println("<li><a href='Products' id='link4'>Products</a></li>");
	...
	try {
         ...
         File file = new File("/home/renuka/eclipse-workspace/MyPro/WebContent/Prod.xml");
		...
		//boolean flaglike=mySqlDataStoreUtilities.checkLike(user);
  		...
  		for (List order: detailsproduct)
  		{
			...
			System.out.println(abc);
  			...
  			if (mySqlDataStoreUtilities.checkRent(user,abc))
			{
			out.println("<form class = 'submit-button' method = 'get' action = 'RentItem'>");
			out.println("<input class = 'submit-button' type = 'submit' name = 'Smartphone_Original' value = 'Rented'");
